...
export default function Results() {
  ...
  const activeCount = filters.advantages.length + filters.activityTypes.length + filters.volunteerProfile.length;
  ...
  const filteredStays = stays.filter((stay) => {
    ...
    const matchesAdvantages =
      ...
      filters.advantages.every((f: string) =>
        stay.accomodations.some((a) => (typeof a === "string" ? a === f : a.label === f))
      );

    const matchesActivity =
      filters.activityTypes.length === 0 || filters.activityTypes.includes(stay.type);
    ...
    return matchesLocation && matchesAdvantages && matchesActivity && matchesProfile;
  });
  ...
  return (
    <SafeAreaView style={{ backgroundColor: COLORS.woofBrown[500] }} className="flex-1" edges={["top"]}>
      ...
      <View className="items-center w-full h-[56px] bg-white flex-row py-4">
        <TouchableOpacity onPress={() => router.back()} className="items-center justify-center ml-6 w-12 h-12">
      ...
      <View className="bg-white p-4 flex-row items-center justify-between border-b border-gray-200">
        ...
        >
          ...
          <View className="ml-2 bg-woofBrown-500 px-2 rounded-full">
            <Text className="text-white text-[12px] font-manropeBold">{activeCount}</Text>
          </View>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView className="flex-1 bg-woofCream-500 px-4">
        <View className="items-center gap-y-6 mt-4">
          {filteredStays.length === 0 ? (
            <Text className="text-gray-500 mt-10 text-[16px]">No stay found for this search.</Text>
          ) : (
