...
export default function Search() {
  ...
  const saveToHistory = async (item: HistoryItem) => {
    try {
      ...
      const newHistory = [item, ...history.filter(h => h.title !== item.title)].slice(0, 10);
  ...
  const filteredStays = useMemo(() => {
    ...
    return stays.filter((s) =>
      `${s.region || ""} ${s.department || ""} ${s.title}`.toLowerCase().includes(query.toLowerCase())
    );
  }, [query, stays]);
      ...
      <ScrollView className="flex-1 bg-white px-4 pt-4">
        {query.length > 0 ? (
          filteredStays.length === 0 ? (
            <Text className="text-center text-gray-500 mt-10">No stay found.</Text>
          ) : (
            filteredStays.map((stay) => (
              <Pressable
                ...
                onPress={() => {
                  saveToHistory({ title: stay.title, region: stay.region || "" });
                  router.push({
                    pathname: "/results",
                    params: {
              ...
              >
                ...
                <View className="ml-4 flex-1">
                  <Text className="text-base font-manropeMedium">{stay.title}</Text>
          ...
          <>
            ...
            {history.map((item, i) => (
              ...
              >
                ...
                <Text className="ml-3 text-[15px] font-manropeMedium">{item.title}</Text>
              </Pressable>
            ))}
          </>
        )}
